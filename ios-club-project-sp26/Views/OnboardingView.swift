...
struct OnboardingView: View {
    ...
    private var stepContent: some View {
        VStack(alignment: .leading, spacing: 24) {
            VStack(alignment: .leading, spacing: 8) {
                ZStack {
                    Circle().fill(AppTheme.purple.opacity(0.15)).frame(width: 52, height: 52)
                    Image(systemName: steps[step].icon).font(.system(size: 22)).foregroundColor(AppTheme.purple)
                }
            ...
            ForEach(mbtiTypes, id: \.self) { type in
                Button { withAnimation(.easeInOut(duration: 0.15)) { mbti = type } } label: {
                    Text(type)
                        ...
                        .overlay(RoundedRectangle(cornerRadius: 10).stroke(mbti == type ? Color.clear : AppTheme.cardBorder, lineWidth: 1))
                }
    ...
    private var bottomButtons: some View {
        HStack(spacing: 12) {
            ...
            .disabled(!canAdvance || service.isLoading)
            .buttonStyle(ScaleButtonStyle())
        }
    }
